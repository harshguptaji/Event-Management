                    ...
                    <div key={event.eventName}>
                        ...
                        <div className='analysis-flex'>
                            {Object.entries(event.statusData).map(([status, statusInfo]) => (
                                <div key={status} className='analysis-inner-container'>
                                    <h4 className='analysis-inner-container-title'>{status} - Total: {statusInfo.count}</h4>
                                    {
                                        statusInfo.count === 0 ? (<p></p>) : (
                                    ...
                                    <ul className='analysis-flex-1'>
                                        {statusInfo.steps.map((step) => (
                                            <li key={step.flowId} className='analysis-flex-1'>
                                                <div className='analysis-flex-1-div'>{step.flowName}</div> 
                                                <div className='analysis-flex-1-div'><Link className='link' to={`/eventflow/${step.flowId}`}>{step.expectedUserNumber}</Link></div>   
                                                <div className='analysis-flex-1-div'>{step.lastUpdatedBy}</div> 
                                            </li>
                                        ))}
                                    </ul>
                                </div>
