...
export const fetchAllTags = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get('https://event-management-backend-gqnh.onrender.com/api/v1/tag/all');  // Backend endpoint to fetch all tags
    ...
    async ({ name, description }, { rejectWithValue }) => {
      try {
        const response = await axios.post(
          'https://event-management-backend-gqnh.onrender.com/api/v1/tag/register',  // Backend endpoint
          { name, description }
        );
...
export const deleteTagById = createAsyncThunk('tags/deleteTagById', async (tagId, { rejectWithValue }) => {
    try {
        const response = await axios.delete(`https://event-management-backend-gqnh.onrender.com/api/v1/tag/delete/${tagId}`);
